...
public class Dc {
    ...
    private static void ContourFaceProc(OctreeNode[] node, int dir, List<Integer> indexBuffer) {
        ...
        if (node[0].Type == OctreeNodeType.Node_Internal || node[1].Type == OctreeNodeType.Node_Internal)
        {
            for (int i = 0; i < 4; i++)
            {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    if (node[order[j]].Type == OctreeNodeType.Node_Leaf || node[order[j]].Type == OctreeNodeType.Node_Psuedo)
                    {
