package survivalGame.world.generation.noise;
...
public class SimplexNoise {
	...
	public static float getNoise(Vector4f pos) {
		return getNoise(new Vector3f(pos.x, pos.y, pos.z));
	}

	public static float getNoise(Vector3f pos) {
		return getNoise(new Vector3f(pos.x, pos.y, pos.z));
	}

	public static float getNoise2(Vector3f pos) {
		float MAX_TERRAIN_HEIGHT = 90000.f;
		...
		return pos.y - (MAX_TERRAIN_HEIGHT * height) + 8000;
	}

	public static byte Sample(Vector3f pos) {
		return (byte) (pos.getY() - Noise(pos) * 8.0f - 8);
	}
...
//		return (float) Cuboid(pos.mul(r));
	...
	public static float Cuboid(Vector3f pos) {
		...
		Vector3f local = pos.subtract(new Vector3f(64 / 2, 64 / 2, 64 / 2));
		Vector3f d = new Vector3f(Math.abs(local.x), Math.abs(local.y), Math.abs(local.z)).subtract(new Vector3f(radius, radius, radius));
		float m = Math.max(d.x, Math.max(d.y, d.z));
		Vector3f max = d;
		return Math.min(m, max.length());
	}

	public static float Sphere(Vector3f pos) {
	...
	// 2D simplex noise
	private static double noise2D(double xin, double yin) {
	...
	public static float BasicFractal(int octaves, float frequency, float lacunarity, float persistence, Vector2f position) {
		float SCALE = 1.0f / 128.0f;
		//Vec2f p = position.mul(NOISE_SCALE);
		Vector2f p = position.mul(SCALE);
		float noise = 0.f;
		...
		for (int i = 0; i < octaves; i++) {
			noise += noise2D(p.x, p.y) * amplitude;
			p = p.mul(lacunarity);
	...
	public static float RidgedMultiFractal(int octaves, float lacunarity, float gain, float offset, Vector2f position) {
		Vector2f p = position.mul(NOISE_SCALE);

		float signal = (float) noise2D(p.x, p.y);
