...
public class TerrainMeshGenerator {
	...
	private static float[] getColors(List<Vertex> verticies) {
		float[] indices = new float[verticies.size() * 3 + 1];
		int vertexPointer = 0;
		for(int i = 0; i < verticies.size(); i++, vertexPointer+=3) {
			Color color = verticies.get(i).getVertexColor();
